...
const Dashboard = () => {
    ...
    return (
        <div className="flex min-h-screen w-full bg-slate-950 text-slate-50 overflow-hidden font-sans">
            ...
            <main className="flex-1 min-w-0 overflow-auto p-12 custom-scrollbar relative bg-[radial-gradient(circle_at_top_right,rgba(30,41,59,0.3),transparent_50%)]">
                {/* Refined Header: Left-Aligned */}
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-16 max-w-7xl mx-auto">
                    <div>
                        ...
                        <p className="text-slate-400 text-lg font-medium tracking-tight">Managing <span className="text-blue-400 font-bold">{jobs.length} open positions</span> and <span className="text-emerald-400 font-bold">{resumeCount} candidates</span>.</p>
                    </div>
                ...
                <div className="max-w-7xl mx-auto space-y-12">
                    {/* Alert Banner */}
                    {newResumeAlert > 0 && (
                        <div className="flex items-center justify-between p-6 bg-blue-600/5 border border-blue-500/20 rounded-3xl text-blue-400 animate-fade-in-up shadow-xl shadow-blue-500/5">
                            <div className="flex items-center gap-4">
                                <div className="size-12 rounded-2xl bg-blue-600/10 flex items-center justify-center border border-blue-500/20">
                                    <Bell className="h-6 w-6 animate-bounce" />
                                </div>
                                <div>
                                    <p className="text-sm font-black tracking-tight text-white mb-0.5">{newResumeAlert} New Submissions</p>
                                    <p className="text-xs text-slate-500 font-medium">Recieved since your last session on {new Date().toLocaleDateString()}.</p>
                                </div>
                            </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-8 px-2">
                            <div className="flex items-center gap-3">
                                <div className="size-10 bg-slate-900 rounded-xl flex items-center justify-center border border-slate-800"><Layers size={20} className="text-blue-500" /></div>
                                <h2 className="text-2xl font-black text-white tracking-tight">Live Listings</h2>
                            </div>
                        ...
                        {loading ? (
                            ...
                            <div className="bg-slate-900/30 border-2 border-slate-800 border-dashed rounded-[3rem] py-32 text-center flex flex-col items-center animate-fade-in-up">
                                ...
                                <h3 className="text-3xl font-black text-white mb-3">No active positions</h3>
                                <p className="text-slate-500 text-lg mb-10 max-w-sm font-medium leading-relaxed italic">"The best talent is out there. Start your hunt by creating a new job listing today."</p>
                                <button onClick={() => navigate('/jobs/create')} className="bg-white text-slate-950 px-10 py-4 rounded-2xl font-black tracking-wide hover:bg-slate-200 transition-all flex items-center gap-3 shadow-2xl shadow-white/5 active:scale-95">
                                    <Plus size={24} /> New Position
                                </button>
                            </div>
                        ) : (
            ...
            {editingJob && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl animate-in fade-in duration-300">
                    <div className="w-full max-w-xl bg-slate-900 border border-slate-800 rounded-[3rem] p-12 shadow-[0_0_100px_-20px_rgba(59,130,246,0.1)] relative">
                        <div className="flex justify-between items-start mb-10">
                            <div>
                                <h3 className="text-3xl font-black text-white tracking-tight mb-2">Edit Requirements</h3>
                                <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">Job: {editingJob.title}</p>
                            </div>
                            <button onClick={closeEditModal} className="p-3 bg-slate-800 hover:bg-slate-700 rounded-2xl transition-colors"><X size={24} /></button>
                        </div>
                        
                        <div className="space-y-8">
                            <div>
                                <label className="block text-[10px] font-black uppercase text-slate-500 tracking-[0.2em] mb-4">Core Competencies</label>
                                <div className="flex flex-wrap gap-2 p-2 min-h-[100px] bg-slate-950 rounded-2xl border border-slate-800 overflow-y-auto custom-scrollbar">
                            ...
                            <div className="flex gap-3">
                                <input type="text" className="flex-1 bg-slate-950 border border-slate-800 rounded-2xl px-6 py-4 text-sm text-white font-bold focus:outline-none focus:border-blue-500 transition-colors" placeholder="Add new skill..." value={newSkill} onChange={e => setNewSkill(e.target.value)} onKeyDown={e => e.key === 'Enter' && addEditSkill()}/>
                                <button onClick={addEditSkill} className="bg-blue-600 hover:bg-blue-500 px-6 rounded-2xl transition-colors text-white font-black">Add</button>
                            </div>

                            <div className="flex gap-4 pt-4">
                                <button onClick={closeEditModal} className="flex-1 px-4 py-4 bg-slate-800 hover:bg-slate-700 rounded-2xl text-sm font-black text-slate-300 transition-all active:scale-95">Discard</button>
                                <button onClick={handleSaveSkills} disabled={editLoading} className="flex-2 px-4 py-4 bg-white hover:bg-slate-200 rounded-2xl text-sm font-black text-slate-950 transition-all active:scale-95">
            ...
            {deleteConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl animate-in fade-in duration-300">
                    <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-[3rem] p-12 shadow-2xl text-center">
                        ...
                        <h3 className="text-3xl font-black text-white mb-4 tracking-tight">Decommission Job?</h3>
                        <p className="text-slate-500 font-medium mb-10 leading-relaxed italic pr-2">"This will permanently archive the position and its candidate rankings. Proceed with caution."</p>
                        <div className="flex gap-4">
                            <button onClick={() => setDeleteConfirm(null)} className="flex-1 px-4 py-4 bg-slate-800 hover:bg-slate-700 rounded-2xl text-sm font-black text-slate-300 active:scale-95 transition-all">Cancel</button>
                            <button onClick={() => handleDeleteJob(deleteConfirm)} className="flex-1 px-4 py-4 bg-red-600 hover:bg-red-500 rounded-2xl text-sm font-black text-white active:scale-95 transition-all shadow-xl shadow-red-500/20">Delete</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl animate-in fade-in duration-300">
                    <div className="w-full max-w-xl bg-slate-900 border border-slate-800 rounded-[3.5rem] p-12 shadow-2xl">
                        <div className="flex items-center justify-between mb-10">
                            <div>
                                <h3 className="text-3xl font-black text-white tracking-tight leading-none mb-2">Import Talent</h3>
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none">Target: {uploadJob.title}</p>
                            </div>
                            <button onClick={closeUploadModal} className="p-3 bg-slate-800 hover:bg-slate-700 rounded-2xl transition-colors"><X size={24}/></button>
                        </div>
                        <form onSubmit={handleFileUpload} className="space-y-8">
                            <label className="group flex flex-col items-center justify-center w-full min-h-[220px] border-4 border-slate-800 border-dashed rounded-[2.5rem] bg-slate-950/50 hover:bg-slate-950 hover:border-blue-500/50 transition-all cursor-pointer relative overflow-hidden">
                                <div className="z-10 flex flex-col items-center justify-center text-center px-10">
                                    <div className="size-16 bg-slate-900 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-blue-600/10 transition-colors border border-slate-800 group-hover:border-blue-500/50">
                                        <Upload className="h-8 w-8 text-slate-500 group-hover:text-blue-500 transition-colors" />
                                    </div>
                                    <p className="text-lg font-black text-white mb-2 leading-none">Drop CVs here</p>
                            ...
                            {uploadFiles.length > 0 && (
                                <div className="bg-slate-950 rounded-3xl p-6 max-h-48 overflow-auto custom-scrollbar border border-slate-800 space-y-3">
                                    <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Pipeline ({uploadFiles.length} items)</h4>
                                    {Array.from(uploadFiles).map((file, i) => (
                                        <div key={i} className="flex items-center justify-between p-3 bg-slate-900 rounded-xl border border-white/5">
                                            <div className="flex items-center gap-3 truncate">
                                                <div className="size-6 bg-blue-600/10 rounded-lg flex items-center justify-center text-blue-500 font-bold text-[10px]">CV</div>
                            ...
                            {uploadStatus && (
                                <div className={`p-6 rounded-2xl flex items-center gap-4 text-sm font-black animate-fade-in-up ${uploadStatus.type === 'success' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shadow-xl shadow-emerald-500/5' : 'bg-red-500/10 text-red-400 border border-red-500/20 shadow-xl shadow-red-500/5'}`}>
                                    {uploadStatus.type === 'success' ? <CheckCircle size={24}/> : <AlertCircle size={24}/>}
                                    {uploadStatus.message}
                                </div>
                            )}
                            
                            <button disabled={uploading || uploadFiles.length === 0} type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-5 px-8 rounded-3xl transition-all shadow-2xl shadow-blue-500/20 active:scale-[0.98] disabled:opacity-30 disabled:grayscale">
